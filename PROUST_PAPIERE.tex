...
pervertidas\footnote{No ensaio ``Imagem de Proust'', a expressão
  ``ligações pervertidas'' (\emph{die pervertierten Bindungen}) é substituída por ``ligações invertidas'' (\emph{die
    invertierte Bindung}). \versal{[N.~O.]}} eram mais úteis do que as normais. Em algum
...
elevada altura da fama e na baixa altura do leito da morte, cai de modo
surpreendente, e, certamente também de modo maravilho, em sua acordo
maçônico no sofrimento, nas suas \emph{défaillances de la
